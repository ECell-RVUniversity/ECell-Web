  ...
  return (
    <main className="events-page">
      <section className="events-hero">
        ...
        <div className="events-feature-wrap"><article className="events-feature"><div className="events-feature-art" style={{ position: "relative" }}><Image src={EVENTS[0].image} alt="Argonyx 2.0" fill sizes="(max-width: 900px) 88vw, 40vw" quality={85} priority loading="eager" /><span>UPCOMING · HACKATHON</span><strong>ARGONYX<br /><em>2.0</em></strong></div><div className="events-feature-bottom"><div><h2>Argonyx<br />2.0</h2><p>18 — 20 SEP 2026 · RV UNIVERSITY, BENGALURU</p></div><b>↗</b></div></article></div>
      </section>
      <section className="events-content" id="calendar">
        ...
        <div className="events-grid">{visibleEvents.map((event) => <article className="events-card" key={event.title}><div className="events-card-image" style={{ position: "relative" }}><Image src={event.image} alt="" fill sizes="(max-width: 700px) 90vw, 45vw" /><span>{event.type}</span></div><div className="events-card-info"><div><h2>{event.title}</h2><p>{event.description}</p></div><time>{event.date === "TBA" ? "DATE" : `${event.date}–${event.status === "UPCOMING" ? "20" : event.date}`}<br />{event.date === "TBA" ? "TBA" : event.month.replace(" 2026", "").replace(" 2025", "")}</time></div></article>)}</div>
